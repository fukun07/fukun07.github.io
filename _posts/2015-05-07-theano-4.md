...
类似于LSTM这样的序列结构在编程时会经常遇到，它们的基本框架是：有一个函数，还有若干序列，我们需要把这个函数反复用到这些序列上（按顺序来）。具体一点，对于LSTM来说，它有三个序列：x、c和h。每一步的操作是固定，就是说我们将函数f反复用于这三个序列上：

$$
(c_{t}, h{t}) = f(x_{t}, c_{t-1}, h_{t-1})
$$
...
$$
(c_{1}, h{1}) = f(x_{1}, c_{0}, h_{0})
$$

\\(x_1\\)是外部输入，而\\(c_0\\)和\\(h_0\\)是不可计算的，需要制定一个初始值才可以将迭代继续下去。一旦迭代开始了：

$$
(c_{2}, h{2}) = f(x_{2}, c_{1}, h_{1})
$$

只需要输入x就行，c和h都能自己计算得到。所以x是每步都需要的，放在*sequences*里面。c和h是每步计算出来，但需要一个初始值来启动，在*outputs_info*里面给初始值就可以了。
